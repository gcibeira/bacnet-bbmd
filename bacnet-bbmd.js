...
class BACnetBBMD extends bacnet {
  constructor(options) {
    ...
    this._bdt = options.bdt || []; //BDT table. e.g. [{address:'192.168.15.10:47809',mask:'255.255.255.255'}, {address:'192.168.30.5',mask:'255.255.255.255'}];
    this._fdt = options.fdt || []; //FDT table. e.g. [{address:'10.8.8.30:47808',TTL:60,Remain:26}, {address:'192.168.50.11',TTL:60,Remain:26}];
  ...
  //Override _receiveData() method from parent, so we can listen to
  //broadcast traffic and call our _handleBroadcast() handler
  ...
  /**
  * Retransmit a forwarded NPDU from another BBMD as broadcast into the local subnet
  * 
  ...
  */
  ...
  /**
  * Wraps a local broadcast message into a Forwarded NPDU including the original sender's address.
  * Retransmits it to each BBMD and Foreign device in the BDT and FDT tables as a unicast message.
  * 
  * @param buffer the content of the message
